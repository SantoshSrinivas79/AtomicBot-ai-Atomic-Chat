...
use app_lib::core::cli::{
    ...
    discover_llamacpp_binary, discover_mlx_binary, download_hf_mlx_model, download_hf_model,
    fetch_hf_gguf_files, fetch_hf_repo_files, init_llamacpp_state, init_mlx_state, list_models,
    ...
    match cmd {
        ...
        ModelsCommands::Download {
            ...
            if !looks_like_hf_repo(&normalized_repo) {
                eprintln!("Error: '{repo}' is not a valid Hugging Face repo ID or URL.");
                std::process::exit(1);
            }
            ...
            match resolved_engine {
                "llamacpp" => {
                    ...
                    let model_id = download_hf_model(
                        &normalized_repo,
                        chosen,
                        tok_ref,
                        move |done, _total| {
                ...
                "mlx" => {
                    ...
                    let dl_pb_clone = dl_pb.clone();
                    let model_id = download_hf_mlx_model(
                        &normalized_repo,
                        &selected_files,
                        tok_ref,
                        move |done, total| {

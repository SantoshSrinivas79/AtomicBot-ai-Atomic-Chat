...
async fn fetch_hf_repo_manifest(
    ...
    if !status.is_success() {
        return Err(match status.as_u16() {
            ...
            404 => format!("HuggingFace repo '{normalized_repo_id}' not found."),
...
pub async fn download_hf_mlx_model(
    ...
    let main_file = files
        ...
        .or_else(|| {
            files
                .iter()
                .find(|file| file.filename.to_lowercase().ends_with(".safetensors"))
        })
        .ok_or_else(|| format!("No safetensors files found in HuggingFace repo '{repo_id}'."))?;
